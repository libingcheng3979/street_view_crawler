...
def bd09lltobd09mc(lng, lat):
    """
    百度坐标系(bd09ll)转百度墨卡托米制坐标系(bd09mc)
    :param lng: 百度坐标系经度
    :param lat: 百度坐标系纬度
    :return: 转换后的墨卡托坐标(x, y)
    """
    # 寻找对应的分带参数
    ...
    # 计算x坐标
    abs_lng = abs(lng)
    x_temp = cF[0] + cF[1] * abs_lng
    x = -x_temp if lng < 0 else x_temp
    
    # 计算y坐标
    abs_lat = abs(lat)
    cC = abs_lat / cF[9]
    y_temp = cF[2] + cF[3] * cC + cF[4] * (cC ** 2) + cF[5] * (cC ** 3) + cF[6] * (cC ** 4) + cF[7] * (cC ** 5) + cF[8] * (cC ** 6)
    y = -y_temp if lat < 0 else y_temp
